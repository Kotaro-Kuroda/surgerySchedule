...
def log_normal_cc(x, U, D, ot):
    return np.log(np.dot(U, x)) - 1 / 2 * sigma(x) + norm.ppf(alpha) * np.sqrt(sigma(x)) - np.log(T + ot)

def grad(x, U, D, ot):
    return np.append(2 * U / (np.dot(U, x)) - 1 / 2 * (D + 2 * U * np.dot(U, x)) / (np.dot(D, x) + np.dot(U, x) ** 2) + norm.ppf(alpha) / 2 * sigma(x) ** (-0.5) * ((np.dot(U, x) * D - 2 * np.dot(D, x) * U) / ((np.dot(D, x) + np.dot(U, x) ** 2) * np.dot(U, x))), - 1 / (T + ot))
...
def objective(model):
    ...
    ob += m9 * sum(model.ost[k] for k in model.K)
    for r in model.R:
        prod = pyo.prod(1 - model.x[s, r] for s in model.S)
        mean = sum(model.x[s, r] * distribution_dict[s.get_group(), 'total_mean'] for s in model.S) + prod * 10 ** (-2)
        variance = sum(model.x[s, r] * distribution_dict[s.get_group(), 'total_variance'] for s in model.S) + prod * 10 ** (-2)
        ob += model.mu[r] * (get_left_hand(alpha, mean, variance) - pyo.log(model.ot[r] + T))
    return ob
...
for d in list_date:
    ...
    while not feasiblity_criteria:
        ...
        model.mu = pyo.Var(model.R, domain=pyo.NonNegativeReals)
