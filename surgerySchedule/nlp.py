...
def rule4(model, r):
    ...
    mean = sum(model.x[s, r] * distribution_dict[s.get_group(), 'total_mean'] for s in model.S) + prod * 10 ** (-2)
    variance = sum(model.x[s, r] * distribution_dict[s.get_group(), 'total_variance'] for s in model.S) + prod * 10 ** (-2)
...
def rule13(model, s, r):
    return model.x[s, r] * (1 - model.x[s, r]) == 0
def rule14(model, s, k):
    return model.q[s, k] * (1 - model.q[s, k]) == 0
...
for d in list_date:
    ...
    while not feasiblity_criteria:
        ...
        model.x = pyo.Var(model.S, model.R, domain=pyo.NonNegativeReals, bounds=(0, 1))
        model.q = pyo.Var(model.S, model.K, domain=pyo.NonNegativeReals, bounds=(0, 1))
        ...
        model.constraint13 = pyo.Constraint(model.S, model.R, rule=rule13)
        model.constraint14 = pyo.Constraint(model.S, model.K, rule=rule14)
        ...
        if l == 1:
            ...
            res = opt.solve(model, mip_solver='cplex', nlp_solver='ipopt', tee=True)
            print(res.solver.termination_condition)
            for s in model.S:
                for r in model.R:
                    x_val[s, r, d, l] = round(model.x[s, r]())
        ...
        res = pyo.SolverFactory("cplex").solve(operating_room_scheduling, tee=True)
        if res.solver.termination_condition == 'infeasible':
            continue
        else:
