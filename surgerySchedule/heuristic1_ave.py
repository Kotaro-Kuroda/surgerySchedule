...
def get_perform_surgeon(surgery, lst_surgeon, lst_date, loop):
    for d in lst_date:
        for surgeon in lst_surgeon:
            if q_val[surgery, surgeon, d, loop] == 1:
                return surgeon
    return 'unscheduled_surgery'
...
for d in list_date:
    ...
    while not feasibility_criteria:
        ...
        for r in list_room:
            operating_room_planning += pulp.lpSum((surgery.get_preparation_mean() + surgery.get_surgery_mean() + surgery.get_cleaning_mean()) * x[surgery, r, d, l] for surgery in waiting_list) - T <= ot[r, d, l]
                ...
                for surgery1 in waiting_list:
                    for surgery2 in waiting_list:
                        if surgery1 != surgery2:
                            operating_room_planning += pulp.lpSum(x[surgery1, r, d, l] for r in list_room if x_val[surgery1, r, d, i] == 1) \
                                                       + pulp.lpSum(x[surgery2, r, d, l] for r in list_room if x_val[surgery2, r, d, i] == 1) \
                                                       + pulp.lpSum(q[surgery1, surgeon, d, l] for surgeon in list_surgeon if q_val[surgery1, surgeon, d, i] == 1) \
                                                       + pulp.lpSum(q[surgery2, surgeon, d, l] for surgeon in list_surgeon if q_val[surgery2, surgeon, d, i] == 1) <= 3
        ...
        operating_room_scheduling += m6 * pulp.lpSum(ot[r, d, l] for r in list_room) + m7 * pulp.lpSum(wt[surgeon, d, l] for surgeon in planned_surgeon) + m8 * pulp.lpSum(surgery.get_priority() * ts[surgery, l] for surgery in planned_surgery)
        for r in list_room:
            list_surgery_in_r = dict_surgery_room[r]
            for surgery1 in list_surgery_in_r:
                for surgery2 in list_surgery_in_r:
        ...
        for r in list_room:
            ...
            for surgery1 in list_surgery_in_r:
                for surgery2 in list_surgery_in_r:
                    if surgery1 != surgery2:
                        operating_room_scheduling += ts[surgery2, l] - surgery2.get_preparation_time() >= ts[surgery1, l] + surgery1.get_surgery_time() + surgery1.get_preparation_time() - M * (1 - y[surgery1, surgery2, r, d, l])
        ...
        for surgeon in planned_surgeon:
            ...
            for surgery1 in list_surgery_by_k:
                for surgery2 in list_surgery_by_k:
                    if surgery1 != surgery2:
                        operating_room_scheduling += ts[surgery2, l] >= ts[surgery1, l] + surgery1.get_surgery_time() + PT - M * (1 - z[surgery1, surgery2, surgeon, d, l])
        ...
        for r in list_room:
            for surgery in dict_surgery_room[r]:
                operating_room_scheduling += msR[r, d, l] >= ts[surgery, l] + surgery.get_surgery_mean() + surgery.get_cleaning_mean()
        ...
        for surgeon in planned_surgeon:
            operating_room_scheduling += wt[surgeon, d, l] >= msS[surgeon, d, l] - tsS[surgeon, d, l] - pulp.lpSum(surgery.get_surgery_mean() * q[surgery, surgeon, d, l].value() for surgery in planned_surgery)
