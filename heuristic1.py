        ...
        for r in list_room:
            ...
            for i in range(len(list_surgery_in_r) * 4):
                ...
                if i % 4 == 0:
                    ...
                    if i == 0:
                        sheet.cell(row=i + 2 + length, column=list_room.index(r) + 2).value = ts_val[surgery] - surgery.get_preparation_mean()
                    else:
                        prev_surgery = list_surgery_in_r[int(i / 4 - 1)]
                        entry_time = ts_val[surgery] - surgery.get_preparation_mean()
                        prev_exit_time = ts_val[prev_surgery] + prev_surgery.get_surgery_mean() + prev_surgery.get_cleaning_mean()
                    ...
                    sheet.cell(row=i + 2 + length, column=list_room.index(r) + 2).value = surgery.get_preparation_mean()
                elif i % 4 == 2:
                    sheet.cell(row=i + 2 + length, column=1).value = "手術" + str(surgery)
                    sheet.cell(row=i + 2 + length, column=list_room.index(r) + 2).value = surgery.get_surgery_mean()
                if i % 4 == 3:
                    sheet.cell(row=i + 2 + length, column=1).value = "清掃" + str(surgery)
                    sheet.cell(row=i + 2 + length, column=list_room.index(r) + 2).value = surgery.get_cleaning_mean()
            ...
            for j in range(len(list_surgery_by_surgeon) * 2):
                ...
                if j % 2 == 0:
                    ...
                    if j == 0:
                        ...
                        sheet.cell(row=j + 2 + length, column=i + 2).value = ts_val[surgery] - (ts_val[prev_surgery] + prev_surgery.get_surgery_mean() + prev_surgery.get_cleaning_mean())
                else:
                    sheet.cell(row=j + 2 + length, column=1).value = "手術" + surgery.get_surgery_id()
                    sheet.cell(row=j + 2 + length, column=i + 2).value = surgery.get_surgery_mean()
        ...
        while not feasibility_criteria:
            ...
            for r in list_room:
                ...
                for surgery1 in list_surgery_in_r:
                    for surgery2 in list_surgery_in_r:
                        if surgery1 != surgery2:
                            operating_room_scheduling += ts[surgery2, l] - surgery2.get_preparation_mean() >= ts[
                                surgery1, l] + surgery1.get_surgery_mean() + surgery1.get_cleaning_mean() - M * (
                    ...
                    for surgery2 in list_surgery_by_k:
                        if surgery1 != surgery2:
                            operating_room_scheduling += ts[surgery2, l] >= ts[
                                surgery1, l] + surgery1.get_surgery_mean() + PT - M * (
            ...
            if pulp.LpStatus[result_status2] == "Infeasible":
                ...
                L.append(l)
                for surgery in planned_surgery:
                    ts_val[surgery] = round(ts[surgery, l].value())
