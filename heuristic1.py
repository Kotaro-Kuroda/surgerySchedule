...
for d in list_date:
    ...
    while not feasibility_criteria:
        ...
        for surgery in waiting_list:
            for r in list_room:
                num_int_var += 1
                x[surgery, r, d, l] = pulp.LpVariable("x({:},{:},{:},{:})".format(surgery.get_surgery_id(), r, d, l),
                                                      cat='Binary')
        ...
        for surgery in waiting_list:
            for surgeon in list_surgeon:
                ...
                q[surgery, surgeon, d, l] = pulp.LpVariable(
                    "q({:},{:},{:},{:})".format(surgery.get_surgery_id(), surgeon.get_surgeon_id(), d, l), cat='Binary')
        for r in list_room:
            num_con_var += 1
            ot[r, d, l] = pulp.LpVariable("ot({:},{:},{:})".format(r, d, l), lowBound=0, cat='Continuous')

        for surgeon in list_surgeon:
            ost[surgeon, d, l] = pulp.LpVariable("ost({:},{:},{:})".format(surgeon.get_surgeon_id(), d, l), lowBound=0,
                                                 cat='Continuous')
        ...
        objective = pulp.lpSum((m1 * (d - surgery.get_release_date()) + m2 * max(d - surgery.get_due_date(), 0)) * x[
        ...
        for surgeon in list_surgeon:
            operating_room_planning += pulp.lpSum(q[surgery, surgeon, d, l] for surgery in waiting_list) <= 3
        for r in list_room:
            operating_room_planning += pulp.lpSum(
                (surgery.get_preparation_time() + surgery.get_surgery_time() + surgery.get_cleaning_time()) * x[
                    ...
                    for surgery2 in waiting_list:
                        if surgery1 != surgery2:
                            operating_room_planning += pulp.lpSum(x[surgery1, r, d, l] for r in list_room if
                                                                  x[surgery1, r, d, i].value() == 1) + pulp.lpSum(
                                x[surgery2, r, d, l] for r in list_room if
                                x[surgery2, r, d, i].value() == 1) + pulp.lpSum(
                                q[surgery1, surgeon, d, l] for surgeon in list_surgeon if
                                q[surgery1, surgeon, d, i].value() == 1) + pulp.lpSum(
                                q[surgery2, surgeon, d, l] for surgeon in list_surgeon if
                                q[surgery2, surgeon, d, i].value() == 1) <= 3

        result_status = operating_room_planning.solve(solver)
        print('ORP={:}'.format(pulp.LpStatus[result_status]))
        for surgery in waiting_list:
            for r in list_room:
                x_val[surgery, r, d] = round(x[surgery, r, d, l].value())
            for surgeon in list_surgeon:
                q_val[surgery, surgeon, d] = round(q[surgery, surgeon, d, l].value())
        planned_surgery = [surgery for surgery in waiting_list if sum(x_val[surgery, r, d] for r in list_room) == 1]
        planned_surgeon = [surgeon for surgeon in list_surgeon if
                           sum(q_val[surgery, surgeon, d] for surgery in planned_surgery) >= 1]
        dict_surgery_room = {}
        for r in list_room:
            list_surgery_in_r = [surgery for surgery in planned_surgery if x_val[surgery, r, d] == 1]
            dict_surgery_room[r] = list_surgery_in_r
        dict_surgery_surgeon = {}
        for surgeon in planned_surgeon:
            list_surgery_by_k = [surgery for surgery in planned_surgery if q_val[surgery, surgeon, d] == 1]
                ...
                for surgery2 in list_surgery_in_r:
                    if surgery1 != surgery2:
                        y[surgery1, surgery2, r, d, l] = pulp.LpVariable(
                            "y({:},{:},{:},{:},{:})".format(surgery1.get_surgery_id(), surgery2.get_surgery_id(), r, d,
                                                            l), cat='Binary')
        ...
        for surgeon in planned_surgeon:
            tsS[surgeon, d, l] = pulp.LpVariable("tsS({:},{:},{:})".format(surgeon.get_surgeon_id(), d, l), lowBound=0,
                                                 cat='Continuous')

        for surgeon in planned_surgeon:
            msS[surgeon, d, l] = pulp.LpVariable("msS({:},{:},{:})".format(surgeon.get_surgeon_id(), d, l), lowBound=0,
                                                 cat='Continuous')

        for surgeon in planned_surgeon:
            wt[surgeon, d, l] = pulp.LpVariable("wt({:},{:},{:})".format(surgeon.get_surgeon_id(), d, l), lowBound=0,
                                                cat='Continuous')
        ...
        for surgery in planned_surgery:
            operating_room_scheduling += ts[surgery, l] >= surgery.get_preparation_time()
        ...
        for r in list_room:
            for surgery in dict_surgery_room[r]:
                operating_room_scheduling += msR[r, d, l] >= ts[
                    surgery, l] + surgery.get_surgery_time() + surgery.get_cleaning_time()

        for surgeon in planned_surgeon:
            for surgery in dict_surgery_surgeon[surgeon]:
                operating_room_scheduling += msS[surgeon, d, l] >= ts[surgery, l] + surgery.get_surgery_time()
        ...
        for surgeon in planned_surgeon:
            operating_room_scheduling += wt[surgeon, d, l] >= msS[surgeon, d, l] - tsS[surgeon, d, l] - pulp.lpSum(
                surgery.get_surgery_time() * q[surgery, surgeon, d, l].value() for surgery in planned_surgery)
        ...
        if pulp.LpStatus[result_status2] == "Infeasible":
            ...
            for surgeon in planned_surgeon:
                ...
                # n_val[surgeon, d] = round(n[surgeon, d, l].value())
            for surgery in planned_surgery:
                for r in list_room:
...
def objective_function(lst_surgery, lst_surgeon, lst_room, lst_date):
    obj_value = sum((m1 * (d - surgery.get_release_date()) + m2 * max(d - surgery.get_due_date(), 0)) * x_val[
        surgery, r, d] * surgery.get_priority() for surgery in lst_surgery for r in lst_room for d in lst_date)
    obj_value += sum((m3 * (surgery.get_due_date() - surgery.get_release_date()) + m4 * (
    ...
    obj_value += m5 * sum(
        ...
        lst_surgery)
    obj_value += m6 * sum(ot_val[r, d] for r in lst_room for d in lst_date)
    obj_value += m7 * sum(wt_val[surgeon, d] for surgeon in lst_surgeon for d in lst_date) + m8 * sum(
