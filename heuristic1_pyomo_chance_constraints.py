...
def log_normal_cc(x, U, D, ot):
    return np.log(np.dot(U, x)) - 1 / 2 * sigma(x) + norm.ppf(0.95) * np.sqrt(sigma(x)) - np.log(480 + ot)

def grad(x, U, D, ot):
    return np.append(2 * U / (np.dot(U, x)) - 1 / 2 * (D + 2 * U * np.dot(U, x)) / (np.dot(D, x) + np.dot(U, x) ** 2) + norm.ppf(0.95) / 2 * sigma(x) ** (-0.5) * ((np.dot(U, x) * D - 2 * np.dot(D, x) * U) / ((np.dot(D, x) + np.dot(U, x) ** 2) * np.dot(U, x))), - 1 / (480 + ot))
